/*
------------------------------------------------------------------------------ 
This code was generated by Amplication. 
 
Changes to this file will be lost if the code is regenerated. 

There are other ways to to customize your code, see this doc to learn more
https://docs.amplication.com/docs/how-to/custom-code

------------------------------------------------------------------------------
  */
import { InputType, Field } from "@nestjs/graphql";
import { PlatformProviderWhereUniqueInput } from "../../platformProvider/base/PlatformProviderWhereUniqueInput";
import { ApiProperty } from "@nestjs/swagger";
@InputType()
class PlatformProviderUpdateManyWithoutTagsInput {
  @Field(() => [PlatformProviderWhereUniqueInput], {
    nullable: true,
  })
  @ApiProperty({
    required: false,
    type: () => [PlatformProviderWhereUniqueInput],
  })
  connect?: Array<PlatformProviderWhereUniqueInput>;

  @Field(() => [PlatformProviderWhereUniqueInput], {
    nullable: true,
  })
  @ApiProperty({
    required: false,
    type: () => [PlatformProviderWhereUniqueInput],
  })
  disconnect?: Array<PlatformProviderWhereUniqueInput>;

  @Field(() => [PlatformProviderWhereUniqueInput], {
    nullable: true,
  })
  @ApiProperty({
    required: false,
    type: () => [PlatformProviderWhereUniqueInput],
  })
  set?: Array<PlatformProviderWhereUniqueInput>;
}
export { PlatformProviderUpdateManyWithoutTagsInput };
